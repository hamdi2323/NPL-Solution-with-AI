"""
====================================================================
Python script to demonstrate NER (Named Entity Recognition)using the
Spacy library.
===================================================================
This script identifies and classifies 'named entities' using
spacy's medium English model into predefined categories such as:
    - PERSON : People's names
    - ORGANIZATION :  Organizations, companies, institutions
    - GPE : Geographies such as( countries, cities, states, provinces and so on)
    - LOCATION : Non-GPE locations (mountains, rivers, waterfall and son on)
...
"""
    ...
    try: return importlib.import_module(module_name)
        ...
        sys.exit(1) #stop further script execution to avoid missing module/library error
...
def load_nlp_model(model_name:str = "en_core_web_md"):
    ...
    try:
        ...
        print(f"[ERROR]: Could not find spaCy model {model_name}"
              f"\n[INFO]: Downloading spaCy model now")
...
def print_summary(doc) -> None:
    """
    Print high-level statistics about NER results
    Includes:
